...
public class Sparkle {
    ...
    public Sparkle(IEventBus modEventBus) {
        ...
        SPARKLY_BLOCKS = CONFIG.blocks.stream()
                ...
                .map(thing -> BuiltInRegistries.BLOCK.getValue(ResourceLocation.parse(thing)))
                .filter(obj -> true)
                .toList();

        SPARKLY_ITEMS = CONFIG.items.stream()
                .filter(string -> string != null && !string.isEmpty())
                .map(thing -> BuiltInRegistries.ITEM.getValue(ResourceLocation.parse(thing)))
                .filter(obj -> true)
                .toList();

        SPARKLY_ENTITIES = CONFIG.entities.stream()
                .filter(string -> string != null && !string.isEmpty())
                .map(thing -> BuiltInRegistries.ENTITY_TYPE.getValue(ResourceLocation.parse(thing)))
                .filter(obj -> true)
                .toList();
    }
